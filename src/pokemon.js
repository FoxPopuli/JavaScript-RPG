...
export class Pokemon {
  ...
  scaleStat(stat, changeFactor) {
    if (stat === "hp") {
      ...
      console.log(
        `${this.prefix + this.name}'s ${stat.toUpperCase() + suffix}`
      );
    }
  }
  ...
  getStats = function () {
    const statObj = {};
    statObj.hp =
      ((2 * this.baseStats.hp + this.ivs.hp + this.evs.hp / 4) * this.level) /
        100 +
      this.level +
      10;
    ...
    for (let stat in this.baseStats) {
      ...
      statObj[stat] =
        (((2 * this.baseStats[stat] + this.ivs[stat] + this.evs[stat] / 4) *
          this.level) /
          100 +
          5) *
        this.nature[stat];
