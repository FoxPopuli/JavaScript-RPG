...
export class Character extends MapObj {
  ...
  processMovement = function (t) {
    ...
    if (t - this.timeMoved >= this.delayMove) {
      ...
      this.position.x =
        this.tileFrom.x * tileW + (tileW - this.dimensions.x) / 2;
      this.position.y =
        this.tileFrom.y * tileH + (tileH - this.dimensions.y) / 2;
  ...
  move = function (currentKey) {
    ...
    // Movement gates
    if (
      this.currentMap.waterMat[nextTile.y][nextTile.x] !== 0 &&
      this.moveType !== "surf"
    )
      return;
  ...
  draw() {
    ...
    ctx.drawImage(
      ...
      this.position.y -
        ...
        this.currentMap.viewport.offset.y +
        offset.y,
...
export class Player extends Character {
  ...
  placeAt = function (x, y) {
    ...
    if (
      this.moveType === "surf" &&
      this.currentMap.waterMat[this.tileFrom.y][this.tileFrom.x] === 0
    ) {
      this.moveType = "walk";
    }
  ...
  switchMon = function () {
    ...
    if (this.isPlayer) {
      ...
      partyIndex = prompt(
        selectionString + "Please make your selection (number): "
      );
    } else {
