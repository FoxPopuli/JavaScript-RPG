...
const viewport = {
  ...
  update: function (px, py) {
    ...
    this.startPixel = {
      x: this.startTile.x * tileW,
      y: this.startTile.y * tileH,
    };
    ...
    if (this.endTile.x >= currentMap.mapFile.width) {
      this.endTile.x = currentMap.mapFile.width - 1;
    }
    if (this.endTile.y >= currentMap.mapFile.height) {
      this.endTile.y = currentMap.mapFile.height - 1;
    }
  },
};
  ...
  if (!currentObj.script.isActive) {
    ...
    switch (e.key) {
      case "e":
        ...
        let objFacing =
          currentMap.objMatrix[player.tileFacing.y][player.tileFacing.x];
    ...
    switch (e.key) {
      case "e":
        if (currentObj.script.menu) {
          currentObj.script.choice =
            currentObj.script.menu.choices[currentObj.script.menu.choiceIndex];
        }
...
function animate() {
  ...
  if (!player.processMovement(currentFrameTime)) {
    ...
    if (
      player.tileFrom.x !== player.tileTo.x ||
      player.tileFrom.y !== player.tileTo.y
    ) {
